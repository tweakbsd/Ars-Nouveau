...
public class SpellResolver {
    ...
    public SpellResolver(SpellContext context){
        this(context.spell.getRecipe(), context);
    }
    ...
    public boolean canCast(LivingEntity entity){
        ...
        for(AbstractSpellPart spellPart : spell.getRecipe()){
    ...
    boolean enoughMana(LivingEntity entity){
        int totalCost = getCastingCost(entity);
    ...
    public int getCastingCost(LivingEntity e){
        int cost = spell.getCost() - getPlayerDiscounts(e);
    ...
    public static void resolveEffects(World world, LivingEntity shooter, RayTraceResult result, Spell spell, SpellContext spellContext){
        ...
        for(int i = 0; i < spell.getRecipe().size(); i++){
